...
my $url = build_url(
  ...
  path     => '/OG2024/data/SCH_ByDisciplineH2H~comp=PG2024.json',
);

captured($ENV{_X_AMZN_TRACE_ID}, $url, sub { paralympic($url) });
...
sub ical
{
  ...
  my $dst = Date::ICal->new(ical => $ical, offset => '+' . $offset)->ical;
